            ...
            <div class="layui-tab-item">
                <table class="layui-table">
                    <tr>
                        <td width="40%">
                            <input type="text" name="account"   autocomplete="off" class="layui-input"
                                   placeholder="根据地址查询交易" id="query_username">
                        </td>
                        <td>
            ...
            <div class="layui-tab-item">
                <table class="layui-table">
                    <tr>
                        <td width="40%">
                            <input type="text" name="account"   autocomplete="off" class="layui-input"
                                   placeholder="请输入合约地址" id="query_contract">
                        </td>
                        <td>
                            <button class="layui-btn query_btn" id="query_contract_btn">查询</button>
                        </td>
                    </tr>
                </table>
                <pre class="layui-code" id="contract_code"></pre>
            </div>
            <div class="layui-tab-item">
                <table class="layui-table">
                    <tr>
                        <td width="10%">
                            <input type="text" name="account"   autocomplete="off" class="layui-input"
                                   placeholder="合约名" id="contract_name">
                        </td>
                        <td width="20%">
                            <input type="text" name="account"   autocomplete="off" class="layui-input"
                                   placeholder="部署人" id="sender_addr">
                        </td>
                        <td width="10%">
                            <button class="layui-btn query_btn" id="contract_deploy_btn">部署</button>
                        </td>
                        <td>
                            <input type="text" name="account"   autocomplete="off" class="layui-input"
                                   placeholder="合约地址" id="contract_addr">
                        </td>
                    </tr>
                </table>
                <textarea name="desc" placeholder="code" class="layui-textarea" id="contract_deploy_code" style="height: 500px"></textarea>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    layui.use(['form', 'jquery', 'element', 'layer', 'table','laypage', 'code'], function(){
        layui.code({
            title: 'Python Code'
        });
        ...
        var user_trans_table = table.render({
            ...
            ,cols: [[{field:'hash', title: '交易哈希'},{field:'height',title: '块高', sort:true}, {field:'block_hash', title: '块哈希'}, {field:'source', title: '付款人'}, {field:'target', title: '收款人'}, {field:'value', title: '金额'}]] //设置表头
            ,data: trans_data
            ,page: true
            ,limit:15
        });
        ...
        $("#sender_addr").val("0xff5a3f5747ada4eaa22f1d49c01e52ddb7875b4b")
        $("#contract_deploy_btn").click(function (){
            let abi = {};
            abi.code = $("#contract_deploy_code").val();
            abi.contract_name = $("#contract_name").val();
            let nonce = new Date().getTime();
            let sender = $("#sender_addr").val()
            let params = {
                "method": "GTAS_t",
                "params": [sender, "", 0, JSON.stringify(abi), nonce],
                "jsonrpc": "2.0",
                "id": "1"
            };
            $.ajax({
                ...
                success: function (rdata) {
                    if (rdata.result !== undefined){
                        $("#contract_addr").val(rdata.result.data)
                    }
                },
            });
        });
        ...
        // 查询交易
        $("#query_contract_btn").click(function () {
            var h = $("#query_contract").val()
            if (h == null || h == undefined || h == '') {
                alert("请输入查询地址")
                return
            }
            let params = {
                "method": "GTAS_contractCode",
                "params": [h],
                "jsonrpc": "2.0",
                "id": "1"
            };
            $.ajax({
                ...
                success: function (rdata) {
                    if (rdata.result !== undefined){
                        $("#contract_code").text(JSON.parse(rdata.result.data).code)
                    }
        ...
        form.on('submit(t_form)', function(data){
            ...
            let params = {
                "method": "GTAS_t",
                "params": [from, to, parseFloat(amount), code, timestamp],
                "jsonrpc": "2.0",
                "id": "1"
            };
        ...
        // 同步块高
        function syncBlockHeight(NUmberUNR) {
            let params = {
            ...
            $.ajax({
                ...
                success: function (rdata) {
                    ...
                    if(NUmberUNR==true){
                        blockCountPages(rdata.result.data)
                    }else{
                        laypage.render({
                            elem: 'demo6'
                            ,count: current_block_height
                            ,limit:15,
                            layout: [ 'prev', 'page', 'next', 'refresh', 'skip','count', ]
                            ,curr:block_detail_currIndex
                            ,jump:function(obj,e){
                                block_detail_currIndex =obj.curr;
                                if(!e){
                                    // console.log(block_detail_currIndexs,obj.curr)
                                    blocks=[] //清空当前数据
                                    blockPagesClick(obj.curr,obj.count);
                                }
                            }
                        });
                        // $('#demo6 .layui-laypage-count').text("共 "+current_block_height +" 条");
                    }
        ...
        // 同步块信息
        function syncBlock(height,indexNumber) {
            let params = {
            ...
            $.ajax({
                ...
                success: function (rdata) {
                    if (rdata.result !== undefined){
                        ...
                        if(blocks.length==15||indexNumber==1){
                            block_table.reload({
        ...
        syncNodes();  // 同步已链接节点
        syncTrans();    // 同步缓冲区交易
        syncBlockHeight(true);  // 同步块高
        syncGroupHeight();  //同步组高
        syncGroup(0)  //同步组信息
        ...
        //查询块分页
        function blockCountPages(count){
            laypage.render({
                elem: 'demo6'
                ,count: count
                ,limit:15,
        ...
        function blockPagesClick(index,current_block_height) {
            for(let i=1; i<=15; i++) {
                jjj = (index-1) * 15 + i;
                if (jjj > current_block_height){
                    return
                }
                if(index==Math.ceil(current_block_height/15)){
                    syncBlock(jjj,1);
                }else{
                    syncBlock(jjj,null);
                }
