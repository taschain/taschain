...
type Database struct {
	diskdb datasource.Database // Persistent storage for matured trie nodes
...
func NewDatabase(diskdb datasource.Database) *Database {
	return &Database{
		diskdb: diskdb,
...
func (db *Database) insert(hash common.Hash, blob []byte) {
	...
	db.nodesSize += StorageSize(common.HashLength + len(blob))
}
...
func (db *Database) dereference(child common.Hash, parent common.Hash) {
	...
	if node.parents == 0 {
		...
		db.nodesSize -= StorageSize(common.HashLength + len(node.blob))
	}
}
