...
//	for _, k := range ag.rctNotifiy.Keys() {
//		t, ok := ag.rctNotifiy.Get(k)
//		if ok {
//			if time.Since(t.(time.Time)).Seconds() > float64(broadcastInterval) {
//				ag.rctNotifiy.Remove(k)
//			}
//		}
//	}
...
//	if t, ok := ag.rctNotifiy.Get(txHash); !ok || time.Since(t.(time.Time)).Seconds() > float64(broadcastInterval) {
//		return true
//	}
//	return false
//}
...
//		for _, tx := range ag.pool.received.asSlice(maxTxPoolSize) {
//			if ag.checkTxCanBroadcast(tx.Hash) {
//				txs = append(txs, tx)
//				if len(txs) >= maxBroadcastPerTime {
//					break
...
//
//	for _, tx := range txs {
//		ag.rctNotifiy.Add(tx.Hash, time.Now())
//	}
//
//	return true
//}
