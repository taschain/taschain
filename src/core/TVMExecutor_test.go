...
`,
	...
	_, _, _, receipts, _ := executor.Execute(state, &block, 0, "")
	...
	executor = TVMExecutor{}
	triedb = account.NewDatabase(db)
	state, err = account.NewAccountDB(common.HexToHash(root.Hex()), triedb)
	if err != nil {
		fmt.Println(err)
	}
	_, _, _, receipts, _ = executor.Execute(state, &block, 0, "")
...
func TestContractCreate(t *testing.T)  {
	...
	triedb := account.NewDatabase(db)
	state, _ := account.NewAccountDB(common.Hash{}, triedb)
	hash, _, _, receipts, _ := executor.Execute(state, &block, 0,"")
	fmt.Println(hash.Hex())
	fmt.Println(receipts[0].ContractAddress.GetHexString())
	root, _ := state.Commit(false)
	fmt.Println(root.Hex())
...
func TestContractCall(t *testing.T)  {
	...
	defer db.Close()
	triedb := account.NewDatabase(db)
	state, err := account.NewAccountDB(common.HexToHash("0xebe99d497383b3f492809715045f0b23324e0b723afd6b1405aa44c2ab6223a0"), triedb)
	if err != nil {
		fmt.Println(err)
	}
	hash, _, _, receipts, _ := executor.Execute(state, &block, 0, "")
