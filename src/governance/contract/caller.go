...
func NewCallContext(bh *types.BlockHeader, bc tasCore.BlockChain, db *account.AccountDB) *CallContext {
	return &CallContext{
		bh:    bh,
		bc:    bc,
		state: db,
...
func infiniteBalance(db *account.AccountDB, account string) common.Address {
	source := common.StringToAddress(account)
	//设置该账户余额
	db.AddBalance(source, new(big.Int).SetUint64(common.MaxUint64))
	return source
}
