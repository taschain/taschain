...
* @return:  
*/
func getRealCode(b *types.Block, db *account.AccountDB, input []byte) ([]byte, error) {
	gov := GetGOV()
...
func voteAddr(tx *types.Transaction, receipts *types.Receipts) common.Address {
...
func handleVoteCreate(callctx *contract.CallContext, b *types.Block, receipts *types.Receipts) error {
	gov := GetGOV()
...
//执行后触发
func afterExecute(b *types.Block, stateDB vm.StateDB, recepts *types.Receipts) error {
	ctx := GetGOV()
