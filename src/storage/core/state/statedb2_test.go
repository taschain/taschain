...
func TestStateDB_SetState(t *testing.T) {
	...
	state.SetState(common.BytesToAddress([]byte("1")), "bb", []byte{1})
	snapshot := state.Snapshot()
	state.SetState(common.BytesToAddress([]byte("1")), "bb", []byte{2})
...
func TestStateDB_GetState(t *testing.T) {
	...
	defer db.Close()
	triedb := NewDatabase(db)
	state, _ := New(common.HexToHash("0xbd51564993c69858d5d2e181fc8d5e5dfdb4e1800f0ead7a1ebdd4a488f5e55f"), triedb)
...
func TestStateDB_GetCode(t *testing.T) {
	...
	defer db.Close()
	triedb := NewDatabase(db)
	state, _ := New(common.HexToHash("0x43b1c4652bd927fce344607f46d61955100dc5b4f358baf2df4f4bfdf2016683"), triedb)
	//fmt.Println(string(state.Dump()))
