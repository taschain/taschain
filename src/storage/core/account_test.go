...
func (s *StateSuite) TestDump(c *checker.C) {
	...
	obj1 := s.state.GetOrNewAccountObject(toAddr([]byte{0x01}))
	obj1.AddBalance(big.NewInt(22))
	obj2 := s.state.GetOrNewAccountObject(toAddr([]byte{0x01, 0x02}))
	obj2.SetCode(sha3.Sum256([]byte{3, 3, 3, 3, 3, 3, 3}), []byte{3, 3, 3, 3, 3, 3, 3})
	obj3 := s.state.GetOrNewAccountObject(toAddr([]byte{0x02}))
	obj3.SetBalance(big.NewInt(44))

	// write some of them to the trie
	s.state.updateAccountObject(obj1)
	s.state.updateAccountObject(obj2)
...
//	s.state.SetData(address, common.Hash{}, value)
//	s.state.Commit(false)
//	value = s.state.GetData(address, common.Hash{})
...
//
//	state.SetData(stateobjaddr0, storageaddr, data0)
//	state.SetData(stateobjaddr1, storageaddr, data1)
//
//	// db, trie are already non-empty values
//	so0 := state.getAccountObject(stateobjaddr0)
...
//	so1 := state.getAccountObject(stateobjaddr1)
...
//
//	so0Restored := state.getAccountObject(stateobjaddr0)
//	// Update lazily-loaded values before comparing.
//	so0Restored.GetData(state.db, storageaddr)
//	so0Restored.Code(state.db)
//	// non-deleted is equal (restored)
//	compareStateObjects(so0Restored, so0, t)
//
//	// deleted should be nil, both before and after restore of state copy
//	so1Restored := state.getAccountObject(stateobjaddr1)
//	if so1Restored != nil {
