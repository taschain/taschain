...
func makeTestState() (AccountDatabase, common.Hash, []*testAccount) {
	...
	for i := byte(0); i < 96; i++ {
		...
		if i%3 == 0 {
			obj.SetCode(common.BytesToHash(sha3.New256().Sum([]byte{i, i, i, i, i})), []byte{i, i, i, i, i})
...
// account array.
func checkStateAccounts(t *testing.T, db tasdb.Database, root common.Hash, accounts []*testAccount) {
	// Check root availability and state contents
	state, err := NewAccountDB(root, NewDatabase(db))
	if err != nil {
...
func checkTrieConsistency(db tasdb.Database, root common.Hash) error {
	if v, _ := db.Get(root[:]); v == nil {
		return nil // Consider a non existent state consistent.
	}
	trie, err := trie.NewTrie(root, trie.NewDatabase(db))
	if err != nil {
		return err
	}
...
func checkStateConsistency(db tasdb.Database, root common.Hash) error {
...
func TestIncompleteStateSync(t *testing.T) {
	...
	checkTrieConsistency(srcDb.TrieDB().DiskDB().(tasdb.Database), srcRoot)
	...
	checkSubtries:
		for _, hash := range added {
			for _, acc := range srcAccounts {
				if hash == common.BytesToHash(sha3.New256().Sum(acc.code)) {
