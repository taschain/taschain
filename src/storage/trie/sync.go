...
func (s *TrieSync) AddSubTrie(root common.Hash, depth int, parent common.Hash, callback LeafCallback) {
	...
	if local, err := decodeNode(key, blob, 0,true); local != nil && err == nil {
		return
	}
...
func (s *TrieSync) Process(results []SyncResult) (bool, int, error) {
	...
	for i, item := range results {
		...
		node, err := decodeNode(item.Hash[:], item.Data, 0,true)
		if err != nil {
...
func (s *TrieSync) Commit(dbw tasdb.Putter) (int, error) {
