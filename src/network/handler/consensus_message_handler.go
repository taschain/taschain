...
//对于每个组成员生成对应的PIECE
type genKeySharePieceFn func() map[groupsig.ID]groupsig.Pubkey

//KEY PIECE 到达监听
type keyPieceArrivedNotifyFn func(kp groupsig.Pubkey, souceId groupsig.ID)

//组用户公钥到达监听
type memberPubkeyArrivedNotifyFn func(mk groupsig.Pubkey, souceId groupsig.ID)

//接收到新的组 本地上链
type addNewGroupToChainFn func(g logical.StaticGroupInfo)

//接收组铸币请求 开始铸币判定和铸币流程
type beginCastFn func()

//组内节点验证接收到的刚铸出来的币
type verifyCastFn func(bh core.BlockHeader, souceId groupsig.ID)

//组内验证过的 块消息监听到达
type verifiedCastMessageArrivedNotifyFn func(bh core.BlockHeader, souceId groupsig.ID)
...
func (h *ConsensusMessageHandler) onMessageKeyPiece(kp groupsig.Pubkey, sd logical.SignData) {
	signResult := h.sv.isNodeSignCorrect(sd)
	if signResult {
		h.keyPieceGot(kp,sd.SignMember)
	}
}
...
func (h *ConsensusMessageHandler) onMessageMemberPubKey(mk groupsig.Pubkey, sd logical.SignData) {
	signResult := h.sv.isNodeSignCorrect(sd)
	if signResult {
		h.memberPubkeyGot(mk,sd.SignMember)
	}
}
...
func (h *ConsensusMessageHandler) onMessageVerifiedCast(bh core.BlockHeader, sd logical.SignData) {
	signResult := h.sv.isNodeSignCorrect(sd)
	if signResult {
		h.castVerifiedGot(bh,sd.SignMember)
	}
}
