...
func (vc *VerifyContext) checkNotify() (*SlotContext) {
	...
	if maxBwSlot != nil {
		blog.log("select max qn=%v, hash=%v, height=%v, hash=%v, size=%v", maxBwSlot.BH.TotalQN, maxBwSlot.BH.Hash.ShortS(), maxBwSlot.BH.Height, maxBwSlot.BH.Hash.ShortS(), len(qns))
	}
	return maxBwSlot
}
