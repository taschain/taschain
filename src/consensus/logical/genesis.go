...
func genGenesisStaticGroupInfo(f string) *genesisGroup {
	...
	group := genesis.Group
	mIndex := make(map[string]int)
	for idx, mem := range group.Members {
		mIndex[mem.ID.GetHexString()] = idx
	}
	group.MemIndex = mIndex
	return genesis
}
