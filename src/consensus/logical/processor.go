...
func (p *Processor) calcCastGroup(preBH *core.BlockHeader, height uint64) *groupsig.ID {
...
//验证块的组签名是否正确
func (p *Processor) verifyGroupSign(b *core.Block, sd SignData) bool {
	bh := b.Header
	var gid groupsig.ID
...
//检查铸块组是否合法
func (p *Processor) isCastGroupLegal(bh *core.BlockHeader, preHeader *core.BlockHeader) (result bool) {
...
func (p *Processor) SuccessNewBlock(bh *core.BlockHeader, vctx *VerifyContext, gid groupsig.ID) {
	begin := time.Now()
	defer func() {
...
//当前节点成为KING，出块
func (p Processor) castBlock(bc *BlockContext, vctx *VerifyContext, qn int64) *core.BlockHeader {
