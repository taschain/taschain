...
// GetMinerPos get a miner's ranking in the group
...
// GetMemberID get the casting block guy at the specified location
...
// GlobalGroup means the parent group node has been announced to the outside world,
// but the group that has not completed initialization is also stored in this structure
//
// Groups that have not been initialized are stored in separate data and are not mixed
// into groups. The choice of the next casting group due to the ranking of the groups
type GlobalGroups struct {
	chain     *core.GroupChain
	groups    []*StaticGroupInfo
	gIndex    map[string]int     // Group index
...
// AddStaticGroup add a legal casting group
//
// When the group is synchronized, the method may be called concurrently,
...
// SelectNextGroupFromCache according to the previous hash value,
// determine which group will cast the next block
